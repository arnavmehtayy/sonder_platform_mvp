...
export function ObjectTreeManager() {
  ...
  const handleCreateDummy = () => {
    ...
    switch (newVariable.type) {
      case "number": typedValue = Number(newVariable.value); break;
      case "string": typedValue = String(newVariable.value); break;
      case "boolean": typedValue = newVariable.value === "true"; break;
    }
  ...
  const handleSaveEdit = (dummyId: number) => {
    ...
    switch (editingVariable.type) {
      case "number": typedValue = Number(editingVariable.value); break;
      case "string": typedValue = String(editingVariable.value); break;
      case "boolean": typedValue = editingVariable.value === "true"; break;
    }
      ...
      <div className="p-4 h-full overflow-y-auto overflow-x-hidden">
        ...
        <div className="bg-white rounded-lg p-3 shadow-sm">
          <Collapsible 
            ...
            onOpenChange={(open) => setOpenSections(prev => ({ ...prev, variables: open }))}
          >
            <CollapsibleTrigger asChild>
              <Button
                variant="ghost"
              ...
              >
                ...
                {openSections.variables ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
              </Button>
            </CollapsibleTrigger>
            <CollapsibleContent>
              <div className="space-y-2">
                {dummyDataObjects.map((dummy) => (
                  <div key={dummy.id} className="p-2 bg-gray-50 rounded-md shadow-sm hover:bg-gray-100">
                    {editingId === dummy.id ? (
                      <div className="space-y-2">
                        <Input
                          placeholder="Name"
                        ...
                        <div className="flex gap-2">
                          ...
                          <div className="flex gap-1 flex-shrink-0">
                            <Button size="sm" onClick={() => handleSaveEdit(dummy.id)} className="h-7 w-7 p-0">
                              <Check className="h-4 w-4" />
                            </Button>
                            <Button size="sm" onClick={() => setEditingId(null)} className="h-7 w-7 p-0">
                              <X className="h-4 w-4" />
                            </Button>
                          </div>
                        </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col min-w-0">
                          <span className="text-sm font-medium truncate">{dummy.name}</span>
                          <span className="text-xs text-gray-500 truncate">Value: {String(dummy.data)}</span>
                        </div>
                        <div className="flex gap-1 flex-shrink-0">
                          <Button
                            variant="ghost"
                ...
                {!showCreateForm ? (
                  ...
                  <div className="space-y-2 mt-4 p-2 bg-gray-50 rounded-md shadow-sm">
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="flex gap-1 flex-shrink-0">
                        <Button size="sm" onClick={handleCreateDummy} className="h-7 w-7 p-0">
                          <Check className="h-4 w-4" />
                        </Button>
                        <Button size="sm" onClick={() => setShowCreateForm(false)} className="h-7 w-7 p-0">
                          <X className="h-4 w-4" />
                        </Button>
                      </div>
                    </div>
