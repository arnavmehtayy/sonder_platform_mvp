...
import {useStore, setInputNumberValueSelector, getControlSelector} from '../store';
...
export default function ShowInputNumber({
  ...
  const value = useStore(state => (state.controls[control_id] as InputNumber).value);
  ...
  return (
    <div className={`bg-white rounded-lg shadow-md p-4 ${!isClickable ? 'opacity-50' : ''} relative`}>
